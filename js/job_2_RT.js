...
function GameLoop() {
	...
	if (spawnTimer <= 0.0 && globalCount < 1)
	{
		...
		var newTarget = {alive:true, timer:0.0, mesh:BABYLON.MeshBuilder.CreateSphere("sphere" + globalCount.toString(), {}, job_1RT2)};
		newTarget.mesh.position = new BABYLON.Vector3(Math.random() * 20.0 - 10.0, Math.random() * 20.0 - 10.0, 0);
		newTarget.mesh.material = new BABYLON.StandardMaterial("texture" + globalCount.toString(), job_1RT2);
	...
	for (var i = 0; i < targets.length; i++)
	{
		if (targets[i].alive)
		{
            ...
            var lifePercent = targets[i].timer / 5000.0;
...
function KeydownListener() {
	...
	//if (evt.keyCode === 32) { // Tecla espaço
	//	gunshot.play();
	//}
...
//     // console.timeEnd('Tempo utilizado --> ');
    ...
    var camera = new BABYLON.DeviceOrientationCamera("DevOr_camera", new BABYLON.Vector3(0, 0, -20), job_1RT2);
    camera.setTarget(new BABYLON.Vector3(0, 0, 0));
    camera.attachControl(canvas, true);

    if (sound)
        var music = new BABYLON.Sound("Violons", "sounds/violons11.wav", job_1RT2, null, { loop: true, autoplay: true });
    //var gunshot = new BABYLON.Sound("Gunshot", "sounds/gunshot.wav", job_1RT2);

    // Add lights to the job_1RT2
    var light1 = new BABYLON.HemisphericLight("light1", new BABYLON.Vector3(1, 1, 0), job_1RT2);
    var light2 = new BABYLON.PointLight("light2", new BABYLON.Vector3(0, 1, -1), job_1RT2);

    //window.addEventListener("keydown", KeydownListener);

    job_1RT2.onBeforeRenderObservable.add(GameLoop);
